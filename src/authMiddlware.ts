...
export const handleTokenExpiration = (req: any, res: Response, next: NextFunction): void => {
    const token = req.header('Authorization')?.replace('Bearer ', '');
    if (!token) {
        res.status(401).send('Access Denied. No token provided.');
        return;
    ...
    try {
        const decoded = jwt.verify(token, 'secretKey', { ignoreExpiration: true }) as UserPayload;
